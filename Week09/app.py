...
# ── Row 3: Class Exercise - Diverging Color Scale Chart ───────────────────────
st.divider()
st.subheader("How much do countries deviate from the global average?")

# Compute deviation from global mean
global_mean = df['Score'].mean()
top_div = filtered.nlargest(top_n, 'Score').copy()
top_div['Score_vs_avg'] = top_div['Score'] - global_mean
top_div = top_div.sort_values('Score_vs_avg')

# BBD COLOUR TYPE: diverging — positive vs negative deviation from global average
# Red indicates below-average happiness, while blue indicates above-average happiness.
fig4 = px.bar(
    top_div, x='Score_vs_avg', y='Country', orientation='h',
    color='Score_vs_avg',
    color_continuous_scale='RdBu',
    color_continuous_midpoint=0,
    labels={'Score_vs_avg': 'Deviation from Global Average Score', 'Country': ''},
)

# Label the midpoint (0) in an annotation
fig4.add_vline(x=0, line_dash="dash", line_color="grey")
fig4.add_annotation(
    x=0, y=len(top_div) - 1 if len(top_div) > 0 else 0,
    text=f"Global Avg Midpoint ({global_mean:.2f})",
    showarrow=True, arrowhead=1, ax=50, ay=30,
    font=dict(color='grey', size=11, family='Arial')
)
...
fig4.update_layout(
    ...
    yaxis=dict(showgrid=False),
    font=dict(family='Arial', size=12),
    coloraxis_colorbar=dict(title='Deviation', thickness=15),
    margin=dict(l=10, r=10, t=40, b=10),
)
